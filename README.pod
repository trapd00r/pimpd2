...
GetOptions(
  ...
  #'aa|add-album'         => sub { @_ = map{$_->file} songs_on_album; _add_files(@_); },
  #'aa|add-album'         => sub { shift; add_album_to_playlist(shift); },
  ...
  'cp'                   => sub { cp(@ARGV ? @ARGV : $config{target_directory}) },
  'cpa|copy-album'       => sub {
  ...
  'lsp|lsplaylist'         => sub { songs_in_playlist(@ARGV); },
